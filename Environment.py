...
class Environment:
    def __init__(self, environment_details, max_steps):
        ...
        points__l = [Point(i) for i in environment_details['points__l']]
        points__r = [Point(i) for i in environment_details['points__r']]
        for i in range(1, len(points__r)):
            pts = [points__r[i-1], points__r[i], points__l[i], points__l[i-1], points__r[i-1]]
    ...
    def point_orientation_wrt_segment(self, point, segment):
        ...
        return True if ((segment[0][0]-point[0])*(segment[1][1]-point[1]) - (segment[1][0]-point[0])*(
                segment[0][1]-point[1])) > 0 else False
    ...
    def point_projection_on_segment(self, point, segment):
        ...
        return (0 <= inner_product) and (inner_product <= (dx*dx+dy*dy))
    ...
    def compute_interaction(self, agents):
        for agent in agents:
            ...
            d = 0
            idx = np.where(np.array(
                [self.check_point_inside_polygon(car_pos, polygon) for polygon in self.polygon_segments]) is True)[0]
            if len(idx) == 1:
                idx = idx[0]
...
def track_generator(track_dict, track_select):
    ...
    for i in range(len(path)):
        ...
        side__l.append((pl.x, pl.y))
        side__r.append((pr.x, pr.y))
    track_dict['points'] = side__r + side__l[::-1]
    track_dict['start_angle'] = start_angle
    track_dict['points_L'] = side__l
    track_dict['points_R'] = side__r
