...
class GUI(object):
    ...
    def init_car(self):
        ...
        for i in range(len(self.cars)):
            ...
            width = self.cars[i]['W']
            length = self.cars[i]['L']
            points = np.array([[0, -width], [length, -width], [length, width], [0, width]]).T
            points = np.dot(r, points)
            points[0, :] += self.cars[i]['state'][0]
            points[1, :] += self.cars[i]['state'][1]
            self.cars[i]['gui_body_id'] = self.display.create_polygon(self.scale_and_offset_center(points.T),
            ...
            for j in range(len(self.cars[i]['sensors'])):
                ...
                points[0, :] += self.cars[i]['state'][0]
                points[1, :] += self.cars[i]['state'][1]
                self.cars[i]['sensors'][j]['gui_sensor_id'] = self.display.create_line(
                    self.scale_and_offset_center(points.T), fill='red')
    ...
    def trace_mod(self, car_id, pt, init=False, force_end_line=False):
        ...
        if len(self.cars[car_id]['trace_history_buffer']) > 100 or force_end_line is True:
            ...
            if len(self.cars[car_id]['trace_history_buffer']) > 2:
                self.cars[car_id]['trace_history'][self.cars[car_id]['trace_history_index']] = self.display.create_line(
                    self.scale_and_offset_center(self.cars[car_id]['trace_history_buffer']), fill='gray65', width=1)
                self.cars[car_id]['trace_history_index'] += 1
                if self.cars[car_id]['trace_history_index'] >= self.trace_history_limit:
                    self.cars[car_id]['trace_history_index'] = 0
            self.cars[car_id]['trace_history_buffer'] = []
    ...
    def update(self, car_id, state, draw_car=True, force_end_line=False):
        ...
        if draw_car is True:
            ...
            width = self.cars[car_id]['W']
            length = self.cars[car_id]['L']
            points = np.array([[0, -width], [length, -width], [length, width], [0, width]]).T
            points = np.dot(r, points)
            points[0, :] += state[0]
            points[1, :] += state[1]
    ...
    def update_graph(self, x, y, plt_idx=None):
        if plt_idx != self.graph_select.get():
            return
        if len(x) < 2:
            return
        min_x, min_y, max_x, max_y = min(x), min(y), max(x), max(y)
        w, h = max_x-min_x, max_y-min_y
        yscale = min_y if h == 0 else float(self.graph_h)/h
        xscale = min_x if w == 0 else float(self.graph_w)/w
        scaled_pts = []
        for (x, y) in zip(x, y):
            scaled_pts.append(int(xscale*(x-min_x)))
            scaled_pts.append(self.graph_h - int(yscale*(y-min_y)))
        self.graph.coords(self.plots[0], *scaled_pts)
        self.graph.itemconfig(self.plots[1], text=self.approximator(min_x)+','+self.approximator(min_y))
        self.graph.itemconfig(self.plots[2], text=self.approximator(max_y))
        self.graph.itemconfig(self.plots[3], text=self.approximator(max_x))
