...
class QLearningNN:
    ...
    def generate_nn(self):
        ...
        activation = None
        self.model.add(keras.layers.Dense(20, kernel_initializer=weights_init, input_shape=(
            self.parameters['state_dimension'],), activation=activation))
        self.model.add(keras.layers.LeakyReLU(alpha=self.parameters['leak_alpha']))
        self.model.add(keras.layers.Dense(self.parameters['output_length'],
                                          kernel_initializer=weights_init, activation=activation))
        self.model.add(keras.layers.LeakyReLU(alpha=self.parameters['leak_alpha']))

        # I found Adam is more stable(than SGD/RMSprop) in handling new samples of (X,y) and overfitting,
        # It does still oscillate but in a more subtle manner
        optim = keras.optimizers.Adam(lr=self.parameters['lr_alpha'],
    ...
    def take_action(self, agent, dt, epsilon_override=None):
        ...
        q_vals = self.model.predict(sensor_readings.reshape(1, self.parameters['state_dimension']),
    ...
    def reward_function(self, agent):
        if agent.state == 'timeup':
            ...
            # reward = 0
            # reward = agent.score # Encourages the car to MOVE, not necessarily forward
            # in fact moving in circles is encouraged
            # reward = -30+agent.score # Encourages the car to crash and end its misery
            # reward = -1 # To factor in time but encourages the car to crash and end its misery.
            # Useful if destination reward is high
    ...
    def train_nn(self, sensor_readings, action, reward, new_sensor_readings, agent_state):
        if len(self.replay) < self.parameters['buffer_length']:
            ...
            self.replay_index += 1
            if self.replay_index >= self.parameters['buffer_length']:
                self.replay_index = 0
        ...
        if len(self.replay) > self.parameters['replay_start_at']:
            ...
            y[non_term_inds, old_actions[non_term_inds].astype(int)] = rewards[non_term_inds] + (
                    self.parameters['gamma'] * max_qs[non_term_inds])
            y[term_inds, old_actions[term_inds].astype(int)] = rewards[term_inds]
            x_train = old_states
            y_train = y
            self.train_hist = self.model.fit(x_train, y_train, batch_size=self.parameters['batchsize'],
                                             epochs=1, verbose=0)
    ...
    def check_terminal_state_and_log(self, agent, env):
        ...
        if agent.state == 'collided' or agent.state == 'destination' or agent.state == 'timeup':
            self.epoch += 1
            if (len(self.replay) >= self.parameters['replay_start_at']) and \
                    self.parameters['epsilon'] < self.parameters['max_epsilon']:
            ...
            if self.epoch % 5 == 0:
                ...
                print('Epoch ', self.epoch, 'Epsilon=', self.parameters['epsilon'], 'Run=', agent.state, 'Avg score=',
                      self.avg_score, 'Avg loss=', self.avg_loss)
            ...
            agent.reset()
            if self.parameters['random_car_position'] is True:
                agent.set_state([1+env.route[0].x+(env.track_width*1.2*(random.random()-0.5)),
                                 env.route[0].y+(env.track_width*1.2*(random.random()-0.5)),
                                 env.start_angle+(random.random()-0.5)])
        return terminal_state, debug_data, self.log['epoch'], self.log['avg_loss'], self.log['final_score'],\
            self.log['cross_score']
